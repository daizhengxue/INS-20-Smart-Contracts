...
emit Inscribe(
  address(0),
  msg.sender,
  string(abi.encodePacked("data:text/plain;charset=utf-8", string(data)))
);
  }
  ...
  abi.encodePacked(
    '{"p":"ins-20","op":"transfer","tick":"',
    _tick,
    '","amt":"',
    string(abi.encodePacked(toString(amount))),
    '"}'
    )
    );
  emit Inscribe(
    from,
    to,
    string(abi.encodePacked("data:text/plain;charset=utf-8", t))
    );
